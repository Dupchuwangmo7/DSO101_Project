...
class BubbleGame extends Phaser.Scene {
  ...
  preload() {
    this.load.image(
      'gun',
      'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="60" height="40" viewBox="0 0 60 40"><rect x="10" y="18" width="40" height="10" rx="4" fill="%231976d2"/><rect x="40" y="14" width="10" height="18" rx="3" fill="%233b82f6"/><rect x="5" y="22" width="10" height="6" rx="2" fill="%234ade80"/></svg>',
    );
  ...
  create() {
    ...
    this._onSetMode = (e: Event) => {
      ...
      if (m === 'add' || m === 'remove' || m === 'addGroup') {
        ...
        if (m === 'addGroup') {
          ...
          this.groupSize =
            typeof gs === 'number' && gs > 0 ? Math.min(10, Math.floor(gs)) : 2;
        }
      }
    };
    ...
    this._onSeedDots = (e: Event) => {
      ...
      try {
        if (this.dots && typeof this.dots.clear === 'function')
      ...
      if (this.mode === 'add') {
        ...
        if (this.dotCount === (this.maxShots || 3)) {
          window.dispatchEvent(
            new CustomEvent('phaser-max-dots', { detail: { answer: 4 } }),
          );
        }
        ...
        this.dotCount =
          (this.dots.getLength?.() as number) || this.dotCount + gs;
        if (this.playLaserSound) this.playLaserSound();
        window.dispatchEvent(new CustomEvent('phaser-dot-created'));
        window.dispatchEvent(new Event('phaser-user-dot'));
      } else {
        const children = (this.dots.getChildren?.() ??
    ...
    const removeAll = () => {
      if (this._onGunToggle)
        window.removeEventListener('phaser-gun-toggle', this._onGunToggle);
      if (this._onClearDots)
        window.removeEventListener('phaser-clear-dots', this._onClearDots);
      if (this._onSetMode)
        window.removeEventListener('phaser-set-mode', this._onSetMode);
      if (this._onSeedDots)
        window.removeEventListener('phaser-seed-dots', this._onSeedDots);
      if (this._onSetMaxShots)
        window.removeEventListener('phaser-set-max-shots', this._onSetMaxShots);
      if (this._onSetDotColor)
        window.removeEventListener('phaser-set-dot-color', this._onSetDotColor);
    };
...
const PhaserBubbleGame: React.FC<PhaserBubbleGameProps> = ({
  onGameEvent,
  lessonStep,
  showAnswerBox,
  onAnswer,
  ...
  useEffect(() => {
    if (
      ...
      window.dispatchEvent(
        new CustomEvent('phaser-set-mode', { detail: { mode: shootMode } }),
      );
    }
  }, [shootMode]);
  ...
  useEffect(() => {
    ...
    const width = Math.max(
      2,
      Math.floor(
        (initialContainer?.clientWidth ?? gameRef.current.offsetWidth) || 800,
      ),
    );
    const height = Math.max(
      2,
      Math.floor(
        (initialContainer?.clientHeight ?? gameRef.current.offsetHeight) || 450,
      ),
    );
  ...
  useEffect(() => {
    ...
    const ro = new ResizeObserver((entries) => {
      ...
      const contW = Math.floor(
        cr.width || el.clientWidth || el.offsetWidth || 0,
      );
      if (!contW || contW < 2) {
      ...
      if (game) {
        ...
        if (newW >= 2 && newH >= 2) {
          ...
          if (scene) {
            scene.width = newW;
            scene.height = newH;
            const bg = scene.children.getByName?.('bg') as
  ...
  useEffect(() => {
    ...
    const onDocDown = (e: MouseEvent) => {
      if (
        ...
        !centerMenuRef.current.contains(e.target as Node)
      ) {
  ...
  const handleGunClick = () => {
    ...
    singleGunTapTimerRef.current = setTimeout(() => {
      setGunSelected((sel) => {
        const next = !sel;
        window.dispatchEvent(
          new CustomEvent('phaser-gun-toggle', { detail: { selected: next } }),
        );
        if (next) {
          const modeToSet =
            doubleShot || shootMode === 'addGroup' ? 'addGroup' : 'add';
          window.dispatchEvent(
            new CustomEvent('phaser-set-mode', {
              detail: {
                mode: modeToSet,
                groupSize: modeToSet === 'addGroup' ? 2 : undefined,
              },
            }),
          );
          setSubGunSelected(false);
    ...
    <>
      ...
      <div
        ...
        style={{
          ...
          overflow: 'hidden',
          background:
            'linear-gradient(140deg, rgba(13,34,60,0.22) 0%, rgba(20,46,80,0.22) 100%)',
        }}
      >
        {/* Overlay Bar */}
        <div
              ...
              {subGunSelected && (
                <span
                  style={{
                    ...
                    borderRadius: '50%',
                    background:
                      'radial-gradient(circle, #fff 60%, #f472b6 100%)',
          ...
          >
            <div
              style={{
                ...
                borderRadius: 9999,
                background:
                  'linear-gradient(120deg, rgba(33,56,96,0.88) 40%, rgba(46,76,128,0.92) 100%)',
            ...
            >
              ...
              <button
                style={{
                  ...
                  borderRadius: '50%',
                  background:
                    'radial-gradient(circle at 60% 35%, rgba(59,130,246,0.35) 0%, rgba(24,26,42,0.85) 70%)',
                  border: '2.5px solid #93c5fd',
                  boxShadow:
                    '0 0 14px #60a5fa88, 0 2px 8px #0007, inset 0 0 10px #60a5fa44',
                  display: 'flex',
                  alignItems: 'center',
                  justifyContent: 'center',
                  position: 'relative',
              ...
              >
                ...
                >
                  ...
                  {/* flame */}
                  <path
                    d="M8 17c2 0 3 2 3 4 0-2 1-4 3-4-2 0-3-1-3-3 0 2-1 3-3 3z"
                    fill="#fca5a5"
                    filter="url(#rocketGlow)"
                  />
                  {/* rocket body */}
                  <path
                    d="M12 3c2.5 1.5 5 5.5 5 9 0 3.5-2.5 5-5 5s-5-1.5-5-5c0-3.5 2.5-7.5 5-9z"
                    fill="#60a5fa"
                    stroke="#ffffff"
                    strokeWidth="1"
                    filter="url(#rocketGlow)"
                  ...
                  {/* fins */}
                  <path
                    d="M9 14l-2 2c.2-1.2.8-2.4 2-2zM15 14l2 2c-.2-1.2-.8-2.4-2-2z"
                    fill="#93c5fd"
                  />
                </svg>
              </button>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <button
              ...
              onClick={() => {
                ...
                setDotColorIdx(next);
                window.dispatchEvent(
                  new CustomEvent('phaser-set-dot-color', {
                    detail: { color: dotColors.current[next] },
                  }),
                );
              }}
        ...
        {showCenterMenu && (
          ...
          >
            ...
            >
              <span>
                Step {typeof lessonStep === 'number' ? lessonStep + 1 : '-'}
              </span>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              <button
                ...
                onClick={() => {
                  setShowCenterMenu(false);
                  if (onGameEvent && typeof lessonStep === 'number')
                    onGameEvent(`step-${lessonStep}`);
                }}
                style={{
                  background:
                    'linear-gradient(90deg, #60a5fa 60%, #38bdf8 100%)',
                  color: '#fff',
              ...
              <button
                ...
                style={{
                  background:
                    'linear-gradient(90deg, #f472b6 60%, #fb7185 100%)',
                  color: '#fff',
