...
function MathPageInner() {
  ...
  if (showEnd) {
    return (
      <div
        className="font-quicksand min-h-screen flex flex-col relative overflow-x-hidden transition-colors duration-500"
        style={{
      ...
      >
        ...
        <div className="flex flex-col items-center justify-center w-full max-w-2xl gap-8 mx-auto">
          <div className="w-full flex flex-col items-center mb-5">
            <div className="text-center text-pink-700 text-lg font-bold mt-0 mb-0 font-[Comic Sans MS,cursive,sans-serif]">
              Level {current + 1} / {questions.length}
            </div>
            <div className="text-center text-blue-500 text-xs mt-0 mb-0 font-semibold">
              Try to get as many correct in a row as you can!
            </div>
            <div className="w-full h-4 bg-pink-100 rounded-full mt-1">
              <div
                ...
                style={{
                  ...
                  background:
                    'linear-gradient(90deg, #fffbe7 0%, #ffe6fa 40%, #b2f7ff 100%)',
                  boxShadow:
                    '0 0 32px 8px #fffbe7cc, 0 0 48px 16px #b2f7ffcc, 0 0 32px 8px #ffe6fa99',
          ...
          {/* Main question card */}
          <Card
            className="puzzle-card p-4 sm:p-8 w-full max-w-2xl min-h-[320px] h-auto flex flex-col items-center justify-center mx-auto overflow-hidden shadow-2xl rounded-[2rem] border-0"
            style={{
              background: 'rgba(255, 255, 255, 0.92)',
              boxShadow:
                '0 0 64px 16px #fffbe7cc, 0 0 96px 32px #b2f7ffcc, 0 0 48px 12px #ffe6fa99',
              border: 'none',
              outline: '3px solid #b2f7ff',
              outlineOffset: '-8px',
              backdropFilter: 'blur(12px)',
              filter:
                'drop-shadow(0 0 32px #fffbe7) drop-shadow(0 0 24px #b2f7ff)',
            }}
          >
            <CardContent className="flex flex-col flex-1 min-h-full w-full h-full items-center justify-center">
              <div className="flex flex-col flex-1 min-h-full w-full h-full items-center justify-center text-center">
                <div className="text-4xl sm:text-5xl font-extrabold text-pink-600 drop-shadow-lg mb-6 font-[Comic Sans MS,cursive,sans-serif]">
                  🎉 You finished all the questions! 🎉
                </div>
                <div className="text-lg text-blue-500 font-semibold mb-8 text-center">
  ...
  const handleSubmit = (e?: React.FormEvent) => {
    ...
    if (q.type === 'input') {
      correct =
        typeof userAnswer === 'string' && userAnswer.trim() === q.answer;
      ...
      correct =
        Array.isArray(userAnswer) && userAnswer.join() === q.answer.join();
  ...
  if (q.type === 'input') {
    ...
    if (q.question && q.question.includes('=')) {
      ...
      questionParts = (
        <span
          className="inline-flex flex-wrap items-center justify-center w-full text-xl sm:text-2xl font-extrabold"
          style={{
            color: '#ff5994',
    ...
    questionContent = (
      ...
      >
        <div
          className="w-full flex-none text-center mb-1"
          style={{ minHeight: '2.5em' }}
        >
          {questionParts || (
            <>
              <span
                className="text-xl sm:text-2xl font-extrabold text-pink-600 drop-shadow-lg font-[Comic Sans MS,cursive,sans-serif] leading-tight"
        ...
        >
          {feedback ? (
            <div
              className={`text-center text-2xl font-extrabold animate-bounce-fast ${feedback.includes('Correct') ? 'text-pink-600' : 'text-blue-700'}`}
              style={{
        ...
        <form
          ...
          className="flex flex-col items-center w-full flex-none mt-0"
        >
          <div
            className="flex flex-col gap-1"
            style={{ transform: 'scale(0.9)', maxWidth: '100%' }}
          >
          ...
          {q.tiles.map((tile, idx) => (
            <div
              ...
              style={{
                ...
                fontFamily:
                  'Fredoka One, Comic Sans MS, Comic Sans, cursive, sans-serif',
    ...
    questionContent = (
      <>
        <div className="mb-6 text-3xl text-center font-extrabold text-blue-500 drop-shadow-lg font-[Comic Sans MS,cursive,sans-serif]">
          Drag to complete the pattern:
        </div>
        <div className="flex gap-3 justify-center mb-4">
          {q.sequence.map((val, i) =>
            val !== null ? (
              <div
                key={i}
                className="w-16 h-16 bg-gradient-to-tr from-blue-200 to-pink-200 text-blue-700 rounded-2xl flex items-center justify-center font-extrabold text-2xl shadow-lg"
              >
                {val}
              </div>
            ) : (
    ...
    const answerArr =
      Array.isArray(userAnswer) && userAnswer.length === q.tiles.length
        ? userAnswer
        : q.tiles;
    ...
    questionContent = (
      <>
        ...
        <div className="flex gap-3 flex-wrap justify-center mb-4 items-center">
          ...
          />
          {answerArr.map((tile: string, idx: number) => (
            <React.Fragment key={tile + idx}>
              <div
          ...
          {q.shapes &&
            q.shapes.map((shape, idx) => {
              ...
              if (shapeName === 'circle') {
                ...
                shapeElem = (
                  <svg width="56" height="56" viewBox="0 0 56 56">
                    <polygon
                      points="28,8 34,24 52,24 37,34 42,50 28,40 14,50 19,34 4,24 22,24"
                      fill="#84ff9f"
                      stroke="#ff5994"
                      strokeWidth="3"
                    />
                ...
                shapeElem = (
                  <svg width="56" height="56" viewBox="0 0 56 56">
                    <polygon
                      points="28,8 50,24 41,48 15,48 6,24"
                      fill="#ff9668"
                      stroke="#ff5994"
                      strokeWidth="4"
                    />
                ...
                shapeElem = (
                  <div className="w-14 h-14 flex items-center justify-center font-bold text-lg bg-gray-200 rounded-xl border-2 border-gray-400">
                    {shapeName}
                  </div>
                );
              }
              return (
                ...
                >
                  {shapeElem}
                  <span className="mt-1 text-xs font-bold text-blue-400 capitalize">
                    {shapeName}
                  </span>
                </button>
              );
